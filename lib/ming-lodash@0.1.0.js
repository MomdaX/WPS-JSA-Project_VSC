class Z_lodash {
    static {
        ...
        /* 铭佬造的轮子 */ {
            const PIVOT_ELEMENTS_FUNCTIONS = {
                identity: (x) => x,
                repivot: (x, index) => {
                    const [rowField, ...rest] = x;
                    if (index == 0) {
                        PIVOT_ELEMENTS_FUNCTIONS.repivot.colFields = rest;
                        return null;
                    }
                    return [rowField, PIVOT_ELEMENTS_FUNCTIONS.repivot.colFields, rest];
                }
            };
            /**
             * 透视表聚合函数集合
             */
            const PIVOT_AGGREGATE_FUNCTIONS = {
                sum: (arr) => {
                    let b = arr.some(Array.isArray);
                    let cb = (a, b) => a + (Number.isFinite(b) ? b : 0);
                    return b ? arr.reduce((x, v) => (x.push(Array.isArray(v) ? v.reduce(cb, 0) : x), x), []) : arr.reduce(cb, 0);
                },
                min: (arr) => {
                    let b = arr.some(Array.isArray);
                    let f = (v) => Math.min(...v.filter(Number.isFinite));
                    return b ? arr.reduce((x, v) => (x.push(Array.isArray(v) ? f(v) : v), x), []) : f(arr);
                },
                max: (arr) => {
                    let b = arr.some(Array.isArray);
                    let f = (v) => Math.max(...v.filter(Number.isFinite));
                    return b ? arr.reduce((x, v) => (x.push(Array.isArray(v) ? f(v) : v), x), []) : f(arr);
                },
                cnt: (arr) => {
                    let b = arr.some(Array.isArray);
                    let f = (a, b) => a + +(b != undefined);
                    return b ? arr.reduce((x, v) => (x.push(Array.isArray(v) ? v.reduce(f, 0) : x), x), []) : arr.reduce(f, 0);
                },
                avg: (arr) => {
                    let b = arr.some(Array.isArray);
                    let f = (v) => {
                        let nums = v.filter(Number.isFinite);
                ...
                first: (arr) => {
                    let b = arr.some(Array.isArray);
                    return b ? arr.map((v) => Array.isArray(v) ? v[0] : v) : arr[0];
                },
                last: (arr) => {
                    let b = arr.some(Array.isArray);
                    return b ? arr.map((v) => Array.isArray(v) ? v[v.length - 1] : v) : arr[arr.length - 1];
                },
                join: (arr) => {
                    let b = arr.some(Array.isArray);
                    return b ? arr.map((v) => Array.isArray(v) ? v.join(',') : v) : arr.join(',');
                },
                unzip_sum: (arr) => {
                    const result = [];
                    for (let j = 0, n; j < arr[0].length; j++) {
                        n = 0;
                        for (let i = 0; i < arr.length; i++) {
                            n += Number.isFinite(arr[i][j]) ? arr[i][j] : 0;
                        }
                        result.push(n);
                    }
                    return result;
             ...
             * 透视表字段排序函数集合
             */
            const PIVOT_FIELD_SORT_FUNCTIONS = {
                asc: (a, b) => {
                    let n;
                    if ((n = a - b) === 0) {
                        return 0;
                    }
                    return n || a?.toString().localeCompare(b.toString()) || 0;
                },
                desc: (a, b) => {
                    let n;
                    if ((n = b - a) === 0) {
                    ...
                    return (+b - +a) || b?.toString().localeCompare(a.toString()) || 0;
                },
            };
            function pivotBy(array, elements = [0, 1, 2], mode) {
                let title, merge_cb, fillVal, agg_mode, options;
                if (typeof mode === 'string') {
                    if ((agg_mode = PIVOT_AGGREGATE_FUNCTIONS[mode]) == undefined) {
                        throw new Error('不支持的聚合方式: ' + agg_mode);
                    }
                }
                else if (typeof mode === 'object') {
                    options = mode;
                    title = options.title;
                    merge_cb = options.merge;
                    agg_mode = options.agg;
                    fillVal = options.fill;
                }
                else if (typeof mode === 'function') {
                    agg_mode = mode;
                }
                const agg_cb = (typeof agg_mode === 'function') ? agg_mode : PIVOT_AGGREGATE_FUNCTIONS[agg_mode];
                let getElements;
                if (Array.isArray(elements)) {
                    const [rKey, cKey, vKey] = elements; // [行字段, 列字段, 值字段]
                    getElements = ele => [ele[rKey], ele[cKey], ele[vKey]]; // 参数归一化, 最终处理成回调函数
                }
                else if (typeof elements === 'string') {
                    getElements = PIVOT_ELEMENTS_FUNCTIONS[elements];
                    if (getElements == undefined) {
                        throw new Error('不支持的确定透视元素方式: ' + elements);
                    }
                }
                    ...
                    throw new TypeError('透视元素类型错误, 只能是数组或约定的字符串或函数');
                }
                const rDic = {}, cDic = {}, res = [[[title]]], mapper = []; // 行字段字典, 列字段字典, 结果数组, 映射数组
                let tmp;
                array.forEach((element, index, source) => {
                    let pivotFields = getElements(element, index, source);
                    if (pivotFields === null) { // 如果明确返回null值, 则跳过该元素(起到筛选作用)
                        return;
                    }
                    let [rKey, cKey, val] = pivotFields; // 回调取得透视元素
                    if (!Reflect.has(rDic, rKey)) {
                        const arr = [];
                        rDic[rKey] = (arr.counter = {}, res.push(arr) - 1);
                    }
                    let i = rDic[rKey];
                    let j = cDic[cKey] ??= res[0][0].push(cKey) - 1;
                    let r = res[i].counter[j] ??= 0;
                    let x = res[i][r] ??= (tmp = [rKey], tmp.memories = [], tmp);
                    if (j in x && !agg_cb && merge_cb) { // 是否发生重复 && 不提供聚合回调 && 合并回调
                        x[j] = merge_cb(x[j], val, rKey, cKey, element, index, source);
                    }
                    else { // 第一次出现
                        agg_cb || merge_cb || ++res[i].counter[j];
                        x[j] = val;
                    }
                    (x.memories[j] ??= []).push(val); // 收集每次出现的值
                });
                let table = [res[0][0]];
                for (let i = 1, colLen = table[0].length, j; i < res.length; ++i) {
                    for (let r = 0; r < res[i].length; ++r) {
                        let h = table.push(res[i][r]) - 1;
                        for (j = 1; j < colLen; ++j) {
                            if (j in res[i][r]) {
                                agg_cb && (table[h][j] = agg_cb(res[i][r].memories[j], res[i][r][0], table[0][j])); // 聚合方式
                            }
                            else {
                                table[h][j] = fillVal; // 填充方式
                            }
                        }
                        delete res[i][r].memories;
                    }
                }
                if (!options) {
                    return table;
                }
                if ('r_sort' in options) { // 是否提供行字段排序方式
                    let cb;
                    if (typeof options.r_sort === 'string') {
                        if ((cb = PIVOT_FIELD_SORT_FUNCTIONS[options.r_sort]) == undefined) {
                            throw new Error('不支持的排序方式: ' + options.r_sort);
                        }
                    }
                    else if (typeof options.r_sort === 'function') {
                        cb = options.r_sort;
                    }
                    else {
                        throw new TypeError('透视表行字段排序方式错误, 只能是约定字符串或函数');
                    }
                    table.sort((a, b) => +(b == table[0]) - +(a === table[0]) || cb(a[0], b[0]));
                }
                if ('c_sort' in options) { // 是否提供列字段排序方式
                    let cb;
                    if (typeof options.c_sort === 'string') {
                        if ((cb = PIVOT_FIELD_SORT_FUNCTIONS[options.c_sort]) == undefined) {
                            throw new TypeError('不支持的排序方式: ' + options.c_sort);
                        }
                    }
                    else if (typeof options.c_sort === 'function') {
                        cb = options.c_sort;
                    }
                    else {
                        throw new TypeError('透视表列字段排序方式错误, 只能是约定字符串或函数');
                    }
                    const hed = table[0].slice(1);
                    const colMap = new Map(hed.map((v, i) => [v, i + 1])); // 记录原列字段顺序
                    const newTable = table.map(v => [v[0]]);
                    hed.sort(cb).forEach((k, j) => {
                        let c = colMap.get(k);
                        table.forEach((_, i) => newTable[i][j + 1] = table[i][c]);
                    });
                    table = newTable;
                }
                if ('r_agg' in options) { // 是否提供每行的聚合方式
                    let r_agg;
                    let r_agg_cb;
                    if (Array.isArray(options.r_agg)) {
                        r_agg = options.r_agg;
                        r_agg_cb = typeof r_agg[1] === 'function' ? r_agg[1] : PIVOT_AGGREGATE_FUNCTIONS[r_agg[1]];
                    }
                    else if (typeof options.r_agg === 'function') {
                        r_agg = ['行总计', options.r_agg];
                        r_agg_cb = options.r_agg;
                    }
                    else if (typeof options.r_agg === 'string') {
                        r_agg = ['行总计', r_agg_cb = PIVOT_AGGREGATE_FUNCTIONS[options.r_agg]];
                    }
                    else {
                        throw new TypeError('透视表行聚合方式错误, 只能是数组或函数');
                    }
                    if (r_agg_cb == undefined) {
                        throw new Error('不支持的聚合方式: ' + r_agg[1]);
                    }
                    table[0].push(r_agg[0]);
                    for (let i = 1; i < table.length; ++i) {
                        table[i].push(r_agg_cb(table[i].slice(1), table[i][0]));
                    }
                }
                if ('c_agg' in options) { // 是否提供每列的聚合方式
                    let c_agg;
                    let c_agg_cb;
                    if (Array.isArray(options.c_agg)) {
                        c_agg = options.c_agg;
                        c_agg_cb = typeof c_agg[1] === 'function' ? c_agg[1] : PIVOT_AGGREGATE_FUNCTIONS[c_agg[1]];
                    }
                    else if (typeof options.c_agg === 'function') {
                        c_agg = ['列总计', c_agg_cb = options.c_agg];
                    }
                    else if (typeof options.c_agg === 'string') {
                        c_agg = ['列总计', c_agg_cb = PIVOT_AGGREGATE_FUNCTIONS[options.c_agg]];
                    }
                    else {
                        throw new TypeError('透视表列聚合方式错误, 只能是数组或函数约定的字符串');
                    }
                    if (c_agg_cb == undefined) {
                        throw new Error('不支持的聚合方式: ' + c_agg[1]);
                    }
                    const lastDatas = [c_agg[0]];
                    for (let j = 1, cols = []; j < table[0].length; ++j) {
                        cols.length = 0;
                        for (let i = 1; i < table.length; ++i) {
                            cols.push(table[i][j]);
                        }
                        lastDatas.push(c_agg_cb(cols, table[0][j]));
                    }
                    table.push(lastDatas);
                }
                let n = options.flatten;
                if (Number.isInteger(options.flatten) && n > 0) { // 是否对透视表每一行的结构进行扁平化处理
                    table = table.map(v => v.flat(options.flatten));
                }
                if ('header' in options && !options.header) { // 是否不包含表头
                    return table.slice(1);
                }
                return table;
            }
            /**
             * 服务于 {@link pivotBy} 的回调集合。
             * @since [2025/8/27] 2.2
             */
            pivotBy.modes = {
                /**
                 ...
                 */
                agg: PIVOT_AGGREGATE_FUNCTIONS,
                /**
                 * 透视表元素确定方式集合。
                 * @remarks 返回的是原始引用, 谨慎修改。
                 */
                elements: PIVOT_ELEMENTS_FUNCTIONS,
                /**
                 * 透视表字段排序方式集合。
                 * @remarks 返回的是原始引用, 谨慎修改。
                 */
                sort: PIVOT_FIELD_SORT_FUNCTIONS
            };
            Object.freeze(pivotBy.modes);

            function to(data, addr, sheet) {
                let sht
                if (typeof sheet == 'string' || typeof sheet == 'number') {
                    try {
                        sht = Worksheets(sheet)
                    } catch {
                        sht = Worksheets.Add(undefined, Worksheets(Worksheets.Count))
                        sht.Name = sheet
                    }
                } else if (sheet == undefined) {
                    sht = ActiveSheet
                } else if (sheet.constructor.name == 'Worksheet') {
                    sht = sheet
                } else {
                    throw new TypeError('sheet参数类型错误')
                }
                const results = [];
                let rowResize, colResize = 1; // 扩展的行列数
                if (typeof data?.[Symbol.iterator] != 'function') {
                    ...
                    colResize = 2;
                    if ((rowResize = results.length) == 0) {
                        throw new TypeError('参数错误, 对象不存在普通成员');
                    }
                }
                else {
                    for (let ele of data) {
                        if (typeof ele != 'string' && typeof ele?.[Symbol.iterator] == 'function') {
                            ele instanceof Array
                                ? results.push(ele)
                                : results.push([...ele]);
                            colResize = Math.max(colResize, results.at(-1).length);
                        }
                        else {
                            results.push([ele]);
                        }
                    ...
                    if ((rowResize = results.length) == 0) {
                        throw new TypeError('参数错误, 不能为空集合');
                    }
                }
                let resizeRange;
                try {
                    resizeRange = sht.Range(addr).Resize(rowResize, colResize);
                    resizeRange.Value2 = results;
                }
                catch (e) {
                    throw new Error(`扩展区域失败! 扩展行数为${rowResize}，扩展列数为${colResize}。${e.message}\n原生Resize方法报错信息: ${e}`);
                }
                return resizeRange
            }

            function log(e, isClear){
                if (isClear) {
                    console.clear()
                }
                if (_.isArray(e)) {
                    console.log(`[\n\t${e.map(s => JSON.stringify(s)).join(',\n\t')}\n]`)
            ...
            function prepend(a, x){
                x = _.isFunction(x) ? x(a) : x
                return a?.[Symbol.iterator] && !_.isString(a) ? [x, ...a] : [x, a]
            }

            function append(a, x){
                x = _.isFunction(x) ? x(a) : x
                return a?.[Symbol.iterator] && !_.isString(a) ? [...a, x] : [a, x]
            }
        ...
        /* lodash库于WPS表格本地化*/{
            ...
            globalThis._ = function (x) {
                return 'raw' in (x || {})
                    ? lodash(Range(x[0])())
                    : lodash(x)
            }
            Object.setPrototypeOf(globalThis._, lodash)
        }
    }
}
